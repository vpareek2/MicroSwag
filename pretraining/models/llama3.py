"""Meta (FAIR) LLaMA 3 as per https://arxiv.org/pdf/2407.21783"""
...
class MLP(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        # Calculate hidden dimension with LLaMA's approach
...
class Block(nn.Module):
    """LLaMA transformer block with pre-normalization"""
    def __init__(self, config):
        super().__init__()
...
class LLaMA(nn.Module):
    ...
    def __init__(self, config):
        ...
        print(f"LLaMA model with {n_params/1e6:.1f}M parameters")
    ...
    def forward(self, idx, targets=None):
        ...
        # Get token embeddings (no position embeddings in LLaMA)
...
def create_llama_from_config(config):
    """Helper function to create a LLaMA model from a config object"""
